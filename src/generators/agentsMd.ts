...
import { generateFeedbackProtocol, USM_UPSTREAM_TRACKER } from "./rulesFiles.js";
...
export function generateRootAgentsMd(
  ...
  lines.push("0. **Confirm the docs watch server is running** (`usm docs serve --watch`).");
  lines.push("   If not running, start it. The human reviews specs in the browser, not just chat.");
  ...
  lines.push("5. **Return the live docs link** so the human can review the rendered spec in the browser. The MCP tool returns `docs_url`/`docs_path` — use those. Docs routes mirror `.usm/features/<area>/<slug>.usm` at `/features/<area>/<slug>` (the `$system` namespace is dropped), served at the port `usm docs serve --watch` reports");
  lines.push("6. Wait for human confirmation that the spec is correct before implementing");
  lines.push("7. Implement the feature in code");
  lines.push("8. Call `usm_update_feature_status` to mark as built (with implementation path)");
  lines.push("");
  ...
  lines.push("5. **Return the live docs link** so the human can verify the updated spec renders correctly");
  lines.push("");
  lines.push("### Generating and Reviewing Docs");
  lines.push("- `usm docs serve --watch` — serve docs locally with VitePress, watch .usm/ for changes");
  ...
  lines.push("- **ALWAYS** return the live docs link after writing a spec — the human reviews in the browser");
  lines.push("- **ALWAYS** wait for human confirmation before implementing");
