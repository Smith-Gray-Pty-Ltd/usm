...
function generateVitePressConfig(root: string, docsRoot: string, audience: Audience = "developer"): string {
  ...
  // Mermaid via CDN with polling — checks every 200ms for unprocessed
  // mermaid code blocks. Replaces <pre><code class="language-mermaid"> with
  // <div class="mermaid"> so mermaid.run() can render them.
  // Dark-mode aware: re-renders on theme toggle.
  const mermaidBoot =
    "(function(){" +
